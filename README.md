...
*Qualitative comparison of the assembled 3D digital twin. \textbf{Left:} The baseline fails to establish a metric space, resulting in floating, unscaled, and overlapping artifacts. \textbf{Right:} With geometric grounding, our method produces a physically plausible, Manhattan-aligned, and tightly registered object-centric scene without subsurface penetration.*
...
Geometric registration metrics and mesh complexity. We report the TrICP Root Mean Square Error (RMSE), \textit{SAM3D}-to-World Scale Factor ($s$), and topological complexity (Vertices and Faces) of the generated meshes for all 12 tracked objects in the scene.
